...
test.describe("Step Figure Hierarchy Admin", () => {
  test("supports create, edit, and delete with component autocomplete", async ({ page }) => {
    ...
    await expect(page.locator(".step-figure-admin__list-item", { hasText: rockName })).toBeVisible();

    // Create base step figure: Shuffle
    await page.getByPlaceholder(/e\.g\. Weave Combination/i).fill(shuffleName);
    await page.getByRole("button", { name: /^Create Step Figure$/i }).click();
    await expect(page.locator(".step-figure-admin__list-item", { hasText: shuffleName })).toBeVisible();
    ...
    await page.getByPlaceholder(/Add component step figure/i).first().fill(rockName);
    await page.getByRole("button", { name: /^Add Component$/i }).first().click();
    await page.getByPlaceholder(/Add component step figure/i).first().fill(shuffleName);
    await page.getByRole("button", { name: /^Add Component$/i }).first().click();
    await page.getByRole("button", { name: /^Create Step Figure$/i }).click();
    ...
    await editSection.locator(".tag", { hasText: rockName }).getByRole("button", { name: "x" }).click();
